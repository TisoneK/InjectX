...
- **Verified commands (all run successfully this session on InjectX):**
  ...
  - **Live backend verification (Session 26):** backgrounded backends (`python backend/main.py &` with `disown` or `setsid`) are killed between Bash tool calls. To verify endpoints live, write a self-contained Python script that uses `subprocess.Popen(..., preexec_fn=os.setsid)` to start the backend, runs every curl/urllib check in-process, and tears down in a `finally` block. The backend stays alive for the duration of the script because the script is the foreground process. Reference implementations: `/home/z/my-project/scripts/verify_phase2.py` (backend curl pass), `/home/z/my-project/scripts/verify_phase2_frontend.py` (Node harness for the frontend API chain — same single-call pattern, loads real `api.js` + a faithful copy of the `main.js` IPC proxies).
  - **dnspython SVCB/HTTPS RR (Session 26):** `params` is a `dns.immutable.Dict` keyed by `ParamKey` enum instances. `str(ParamKey.ECH)` returns `"5"` (the IANA numeric value), NOT `"ech"` — match on `k.name.lower() == "ech"` instead. The value for the `ech` key is an `ECHParam` object; call `.to_text()` to get the base64 string. `ParamKey` is in `dns.rdtypes.svcbbase` (not `dns.rdtypes.IN.SVCB` as you might guess from the import path).
- **Quirks:**
...
---
## Local macOS dev machine (last verified 2026-08-03)
...
- **Verified commands (all run successfully this session on InjectX):**
  ...
  - **Sync + initialize smoke (Session 31):** after `context-sync update` to core 0.5.0, baseline is `python -m pytest -q` → **166 passed**, `ruff check .` → clean, `node --check` clean on all 4 frontend JS files, all with the existing venv + node_modules (no reinstall needed — deps already current). Live boot: `INJECTX_PORT=8799 INJECTX_UPLOAD_DIR=/tmp/injectx-init/uploads python main.py` then `curl /api/health` → `{"status":"ok","version":"0.4.0","ir_version":"1.0"}`; `/api/formats`, `/api/sni/seedlists` 200; `GET /api/config/parse?filepath=.../assets/configs/hc/bypass.hc` → decryption_status=success, scheme A5. **Re-verified kill trap:** `kill $PID` (SIGTERM) does NOT stop the uvicorn dev server — it kept LISTENing on 8799; `kill -9` + `lsof -iTCP:<port> -sTCP:LISTEN -n -P` required. Use `kill -9` from the start on this machine.
  - **Kickoff + baseline verification (Session 32, 2026-08-03):** `git pull --ff-only` was already up to date; `sh .context/core/bin/context-sync verify` passed; `context-sync status` reported core 0.5.0 and source 0.5.0 up to date. Existing `backend/.venv` ran `python -m pytest -q` → **166 passed in 3.49s**, `ruff check .` → clean, all four frontend files passed `node --check`, and `npm audit --omit=dev --audit-level=high` reported 0 vulnerabilities. Node modules were already present, so no install was needed.
  - **Session 33 live recheck (2026-08-03):** on scratch port 8799, backend `/api/health` returned `status=ok`, `/api/formats` served 11 entries, `/api/sni/seedlists` served 3 lists, and `assets/configs/hc/bypass.hc` parsed successfully as HC. Exact backend PID was terminated with `kill -9` and the scratch port was released.
